    ...
    def render_knowledge_screen(self, *, topic: str) -> FlowResult:
        if topic == "guide":
            text = screen_text(
                ...
                lines=[
                    (
                        "Купилка помогает безопасно выполнить покупку на Wildberries, подтвердить заказ "
                        "и отзыв через расширение Qpilka, а после разблокировки вывести кэшбэк в USDT."
                    ),
                    (
                        ...
                        "3. Нажмите «Купить»: бот забронирует товар и покажет токен заявки на покупку.\n"
                        "4. Вставьте токен в расширение Qpilka и следуйте подсказкам для оформления заказа. "
                        "Это нужно сделать в течение 4 часов, иначе бронь отменится.\n"
                        "5. После оформления заказа расширение выдаст токен-подтверждение. Отправьте его в бот.\n"
                        "6. Выкупите товар.\n"
                        "7. Через расширение Qpilka оставьте отзыв на 5 звезд с нужными фразами, "
                        "если они указаны в инструкции к покупке.\n"
                        "8. Отправьте в бот токен-подтверждение отзыва.\n"
                        "9. Дождитесь разблокировки кэшбэка через 15 дней после выкупа.\n"
                        "10. Когда кэшбэк станет доступен, откройте «Баланс и вывод» "
                        "и оформите вывод на адрес USDT в сети TON."
                    ),
                    (
                        "<b>FAQ</b>\n"
                        "1. <b>Где найти товар?</b>\n"
                        "Ссылки на конкретные товары публикуются в профильных телеграм группах.\n\n"
                        "2. <b>Что, если заказ не был сделан в течение 4 часов?</b>\n"
                        "Бронь отменится. Оформите новую покупку, если товар еще доступен.\n\n"
                        ...
                        "4. <b>Что будет, если я не выкуплю товар или верну его?</b>\n"
                        "Кэшбэк выплачен не будет.\n\n"
                        "5. <b>Где гарантия, что продавец выплатит кэшбэк?</b>\n"
                        "Сервис обеспечивает выплату: деньги заранее замораживаются на балансе продавца.\n\n"
                        "6. <b>Как подготовить кошелек для вывода?</b>\n"
                        "Откройте «Про баланс и вывод»: там описано, как создать Крипто Кошелек "
                        "в Telegram и какой адрес USDT TON вставлять в заявку."
                    ),
                    (
                        "<b>Про суммы</b>\n"
                        ...
                        "Точная сумма в USDT показывается на экранах вывода."
                    ),
                ],
                separate_blocks=True,
            )
            ...
            text = screen_text(
                ...
                lines=[
                    ...
                    (
                        ...
                        "1. Забронировать товар в боте.\n"
                        "2. Оформить заказ через расширение Qpilka и отправить токен-подтверждение "
                        "в течение 4 часов.\n"
                        "3. Выкупить товар.\n"
                        "4. Через расширение Qpilka оставить отзыв на 5 звезд с нужными фразами, "
                        "если они указаны.\n"
                        "5. Отправить токен-подтверждение отзыва в бот.\n"
                        "6. Подождать 15 дней после выкупа для разблокировки кэшбэка.\n"
                        "7. Оформить вывод в разделе «Баланс и вывод»."
                    ),
                    (
                        "<b>Полезно знать</b>\n"
                        "1. От покупки можно отказаться, пока она еще не подтверждена.\n"
                        "2. Один и тот же товар нельзя брать повторно с одного аккаунта.\n"
                        "3. Если отзыв не подтвержден, кэшбэк останется заблокированным.\n"
                        "4. Если покупка уже идет, возвращайтесь к ней через раздел «Покупки»."
                    ),
                ],
                separate_blocks=True,
            )
            ...
            text = screen_text(
                ...
                lines=[
                    (
                        "На балансе покупателя отображается только кэшбэк, который уже доступен для вывода, "
                        "и сумма, которая сейчас находится в заявке на вывод."
                    ),
                    (
                        "Суммы на обычных экранах обычно показываются приблизительно в ₽ для удобства. "
                        "Точная сумма вывода и все операции внутри системы ведутся в USDT."
                    ),
                    (
                        "<b>Когда можно выводить</b>\n"
                        "1. Кэшбэк становится доступен после выкупа, подтверждения отзыва "
                        "и окончания срока разблокировки.\n"
                        "2. Если доступного баланса нет, кнопки вывода скрыты.\n"
                        "3. Может быть только одна активная заявка на вывод. Пока она обрабатывается, "
                        "новую создать нельзя."
                    ),
                    (
                        "<b>Как подготовить кошелек в Telegram</b>\n"
                        '1. Откройте официальный Крипто Кошелек в Telegram: <a href="https://t.me/wallet">@wallet</a>.\n'
                        "2. Запустите мини-приложение и создайте аккаунт. "
                        "Кошелек может попросить пройти идентификацию или проверку.\n"
                        "3. Чтобы получить адрес, откройте в Крипто Кошельке: "
                        "«Крипто» → «Пополнить» → «Стейблкоины» → «USDT» → «TON».\n"
                        "4. Скопируйте адрес для пополнения в сети TON. Именно этот адрес нужно вставлять в Купилку.\n"
                        "5. Не используйте адреса TRC-20, ERC-20, Solana или testnet: "
                        "заявка на вывод должна быть только на USDT в сети TON mainnet."
                    ),
                    (
                        "<b>Как оформить вывод в Купилке</b>\n"
                        "1. Откройте «Баланс и вывод».\n"
                        "2. Нажмите «Вывести все доступное» или «Указать сумму вручную».\n"
                        "3. Если выбрали ручной ввод, отправьте сумму в USDT.\n"
                        "4. Вставьте адрес USDT в сети TON.\n"
                        "5. Бот проверит адрес через TonAPI и создаст заявку.\n"
                        "6. После создания заявки статус будет «на проверке у администратора»."
                    ),
                    (
                        "<b>Что происходит после заявки</b>\n"
                        "1. Сумма заявки переходит из «Доступно для вывода» в «В процессе вывода».\n"
                        "2. Пока заявка не отправлена, ее можно отменить. Средства вернутся в доступный баланс.\n"
                        "3. Когда вывод отправлен, статус и tx hash появятся в «Транзакциях».\n"
                        "4. Если заявка отклонена, средства вернутся в доступный баланс, "
                        "и можно создать новую заявку."
                    ),
                    (
                        "<b>Что делать после зачисления USDT в Wallet</b>\n"
                        "1. Когда вывод из Купилки поступит в Крипто Кошелек, откройте: Крипто → Вывести.\n"
                        "2. Выберите один из способов вывода и следуйте инструкции в Wallet.\n"
                        '3. Описание способов вывода: <a href="https://help.ru.wallet.tg/article/68-vyvesti">'
                        "https://help.ru.wallet.tg/article/68-vyvesti</a>."
                    ),
                    (
                        "<b>Важно</b>\n"
                        "1. Проверяйте адрес перед отправкой: переводы в блокчейне нельзя отменить после отправки.\n"
                        "2. Купилка отправляет USDT только в сети TON.\n"
                        "3. Если вывод задержан или отклонен, откройте «Транзакции» "
                        "и используйте номер заявки вида W123 при обращении в поддержку."
                    ),
                ],
                separate_blocks=True,
            )
