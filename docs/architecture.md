...
- `services/bot_api/seller_marketplace_flow.py`: transport-neutral seller marketplace flow for dashboard, knowledge
  screens, shop management, announcement management, seller balance, collateral top-up, and transaction history screens;
  it owns seller copy, prompts, button descriptors, and domain-error mapping while the Telegram runtime bootstraps sellers
