...
- Payload integrity/signature scheme for extension tokens.
- WB correction operation semantics (`Коррекция продаж`, `Коррекция возвратов`).
...
- Token-at-rest cryptography hardening:
  ...
  - status: intentionally deferred; current implementation is an accepted risk.
...
- Runtime secret strictness hardening:
  ...
  - status: intentionally deferred; current defaults are accepted only for local/dev-oriented paths.
