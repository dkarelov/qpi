...
support_bot_telegram_get_me() {
  ...
  echo "Telegram getMe failed through TELEGRAM_API_PROXY_URLS. Set QPI_ALLOW_DEPLOY_WHEN_TELEGRAM_UNREACHABLE=1 only for intentional emergency deploys." >&2
  return 1
}
...
if [[ "${mode}" == "support-bot" ]]; then
  printf 'support_bot_database_url_set=%q\n' "true"
fi
