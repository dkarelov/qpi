...
Optional environment:
  ...
  TELEGRAM_UPDATE_MODE (default: polling; webhook requires BOT_WEBHOOK_SECRET_TOKEN)
  BOT_WEBHOOK_SECRET_TOKEN (required only when TELEGRAM_UPDATE_MODE=webhook)
...

{
  ...
  echo "TOKEN_CIPHER_KEY=${TOKEN_CIPHER_KEY}"
  echo "TELEGRAM_UPDATE_MODE=${TELEGRAM_UPDATE_MODE}"
  echo "YC_FOLDER_ID=${YC_FOLDER_ID}"
  echo "ADMIN_TELEGRAM_IDS=${ADMIN_TELEGRAM_IDS:-}"
  echo "SUPPORT_BOT_USERNAME=${SUPPORT_BOT_USERNAME:-}"
  echo "TELEGRAM_API_PROXY_URLS=${TELEGRAM_API_PROXY_URLS}"
  if [[ -n "${BOT_WEBHOOK_SECRET_TOKEN:-}" ]]; then
    echo "WEBHOOK_SECRET_TOKEN=${BOT_WEBHOOK_SECRET_TOKEN}"
  fi
} > "${overrides_env}"
...
QPI_ALLOW_DEPLOY_WHEN_TELEGRAM_UNREACHABLE=${QPI_ALLOW_DEPLOY_WHEN_TELEGRAM_UNREACHABLE}
EOF
