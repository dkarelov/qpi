...
- Runner-touching concurrency is scoped to DB/schema/preflight runner jobs, not whole workflows. Final marketplace runtime/function rollout jobs are intentionally not mutually serialized when schema is already clean; this assumes the private runner remains single-slot or concurrent runtime/function rollout remains target-safe.
